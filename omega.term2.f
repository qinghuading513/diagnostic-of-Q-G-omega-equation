     ...
     &status='unknown',access='sequential')
      ...
      open (13,file='tempt.8020.daily.37level.seq',
     &form='unformatted',
     &status='unknown',access='sequential')

      open (15,file='term2.8020.monthly.37level.seq',
     &form='unformatted',
     &status='unknown',access='sequential')
...
C       this is the output field for monthly term2 values
...
C       read in daily geopotential height (uu, south to north) 
C       and temperature (vor, north to south)
C       fields at each vertical level:

      read(11)((uu(i,j),i=1,im),j=1,jm)
      read(13)((vor(i,j),i=1,im),j=jm,1,-1)
...
C       calculate the gradient of temperature (u1, v1, north to south)

      call gradient(vor,u1,v1,im,jm)

C        calcaulte term2 (t) on a daily basis ( north to south)    
...
C       calculate monthly mean value of term2: a (north to south)
...
C       output monthly term2 values at 37 vertical levels ( from south
C       to north)
